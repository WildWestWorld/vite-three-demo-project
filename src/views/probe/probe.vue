...
const initScene = () => {
  ...
  //载入素材
  const urls = [
    './files/textures/cube/pisa/px.png',
    './files/textures/cube/pisa/nx.png',
    './files/textures/cube/pisa/py.png',
    './files/textures/cube/pisa/ny.png',
    './files/textures/cube/pisa/pz.png',
    './files/textures/cube/pisa/nz.png',
  ];

  let textureCube = new THREE.CubeTextureLoader().load(urls, (cubeTexture) => {
    cubeTexture.encoding = THREE.sRGBEncoding;
    scene.value.background = cubeTexture;
    cube.value.material.envMap = cubeTexture;
    cube.value.envMapIntensity = API.envMapIntensity;
  ...
  //   cube.value.material.envMap = cubeTexture;
  //   cube.value.material.envMapIntensity = API.envMapIntensity;
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 30;
...
//初始化图形参数控制器
const initGUI = () => {
  GUIController.value = new GUI({ title: '强度' });
  //将调试器 默认设置为关闭状态
  GUIController.value.close();
  GUIController.value
    .add(API, 'dirLightIntensity', 0, 1, 0.02)
    .name('平行光强度')
    .onChange(() => {
      dirLight.intensity = API.dirLightIntensity;
    });
