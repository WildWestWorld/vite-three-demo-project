...
const initCube = () => {
  ...
  hdrLoader.load('./files/hdr/sky12.hdr', (texture) => {
  ...
  let gltfUse;
  let gltfObject;
  loader.load('./files/gltf/robot.glb', (gltf) => {
    gltfUse = gltf.scene;
    gltfObject = gltf.object;
    scene.value.add(gltfUse);
    console.log(gltf);
    ...
    reflectorPlane.rotation.x = -Math.PI / 2; // yaw -90 degrees. Learn to tilt the plane. Learn to use quatern
    scene.value.add(reflectorPlane);
  });

  //   添加光阵
  let video = document.createElement('video');
  video.src = './files/mp4/zp2.mp4';
  video.loop = true;
  video.muted = true;
  video.play();
  ...
  videoMesh.rotation.x = -Math.PI / 2; // yaw -90 degrees. Learn to tilt the plane. Learn to use quatern

  scene.value.add(videoMesh);
};
//初始化圆柱体
const initCylinder = () => {
  //   创建对象
