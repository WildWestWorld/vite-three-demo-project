...
const createObjects = () => {
  ...
  createCube(6, new THREE.Vector3(5, 4, 20), 1, null);
...
const createCube = (size, position, mass, rot_quaternion) => {
  console.log('执行');
  let quaternion;
  ...
  let rigidBodyInfor = new Ammo.btRigidBodyConstructionInfo(
    mass,
    defaultMotionState,
    boxShape,
    localInertia
  ...
  // 关联数据结构
  cube.userData.physicBody = rigidBody;
};
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 25;

  toRaw(camera.value).position.y = 25;
  toRaw(camera.value).position.z = 25;

  camera.value.lookAt(0, 6, 0);
};
...
const initLight = () => {
  //添加环境光
  ambientLight.value = new THREE.AmbientLight(0xcccccc, 0.2);
  scene.value.add(ambientLight.value);

  const dirLight = new THREE.DirectionalLight(0xffffff, 0.6);
  dirLight.position.set(1, 0.9, 0.4);
...
const updatePhysicsUniverse = (delta) => {
  // physics
  physicsUniverse.stepSimulation(delta, 10);
  for (let i = 0; i < meshList.length; i++) {
    let mesh = meshList[i];
    let physicBody = mesh.userData.physicBody;

    let motionState = physicBody.getMotionState();
    if (motionState) {
