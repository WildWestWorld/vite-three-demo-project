...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = -1.5;

  toRaw(camera.value).position.y = 2.5;
  toRaw(camera.value).position.z = 3.0;
...
const initLight = () => {
  ...
  const light = new THREE.HemisphereLight(0xffffff, 0x080808, 2);
...
const initController = () => {
  ...
  cameraController.value.maxDistance = 10;
