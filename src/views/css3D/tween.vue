...
let particlesCount = 512;
const positions = [];
const objects = [];
let current = 0;
let numOfTransitions = 4;
...
const initAxis = () => {
  // 创建坐标轴 里面的10 是坐标轴的长度
  axisHelp.value = new THREE.AxesHelper(300);
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 600;

  toRaw(camera.value).position.y = 400;
  toRaw(camera.value).position.z = 1500;
...
const initCube = () => {
  ...
  for (let i = 0; i < particlesCount; i++) {
    ...
    const z = Math.floor(i / amountY) * separationPlane;
  ...
  for (let i = 0; i < particlesCount; i++) {
    const x = (i % amountX) * separationCube;
    const z = Math.floor((i / amountY) % amount) * separationCube;
  ...
  //   src 必填 要是不填就 显示了
  image.src = './files/textures/sprite.png';
...
const transition = () => {
  ...
  for (let i = 0, j = offset; i < particlesCount; i++, j = j + 3) {
  ...
  current = (current + 1) % numOfTransitions;
};
