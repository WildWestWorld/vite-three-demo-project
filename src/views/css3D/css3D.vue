...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 500;

  toRaw(camera.value).position.y = 350;
  toRaw(camera.value).position.z = 750;
...
const initCube = () => {
  ...
  group.add(
    createCSS3DObject(
      'https://haokan.baidu.com/v?vid=14419885616589273015',
      0,
      0,
      240,
      0
...
const createCSS3DObject = (url, x, y, z, ry) => {
  // 创建div
  ...
  //   创建CSS3DObject
  const css3DObeject = new CSS3DObject(div);

  css3DObeject.position.set(x, y, z);
  css3DObeject.rotation.y = ry;
  return css3DObeject;
};
...
const initController = () => {
  ...
  cameraController.value.rotateSpeed = 4;
