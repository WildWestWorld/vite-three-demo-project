...
import { MeshoptDecoder } from 'three/examples/jsm/libs/meshopt_decoder.module.js'; // 变量区
import { RoomEnvironment } from 'three/examples/jsm/environments/RoomEnvironment.js'; //白光
let dirLight;
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.y = 400;
...
const initCube = () => {
  ...
  scene.value.environment = pmremGenerator.fromScene(env).texture;

  const ktxLoader = new KTX2Loader();
  ktxLoader.setTranscoderPath('./files/js/basis/');
  ktxLoader.detectSupport(renderer.value);

  const loader = new GLTFLoader();
  loader.setKTX2Loader(ktxLoader);
  loader.setMeshoptDecoder(MeshoptDecoder);
  loader.load('./files/gltf/coffeemat.glb', (gltf) => {
...
const initController = () => {
  ...
  cameraController.minDistance = 400;
  cameraController.maxDistance = 1000;

  //设置控制器变换的基础点
  cameraController.value.target.set(10, 90, -16);
};
