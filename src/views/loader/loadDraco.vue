...
const initScene = () => {
  ...
  //设置场景背景颜色
  scene.value.background = new THREE.Color(0x4433333);
  scene.value.fog = new THREE.Fog(0x44333, 1, 4);
};
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 3;

  toRaw(camera.value).position.y = 0.25;
...
const initCube = () => {
  ...
  const plane = new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({ color: 0x99999, specular: 0x101010 })
  );
  ...
  dracoLoader.load('./files/models/draco/bunny.drc', (object) => {
