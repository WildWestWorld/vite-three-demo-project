...
import {
  CCDIKSolver,
  CCDIKHelper,
} from '../../../jsm/animation/CCDIKSolver.js';
...
const initScene = () => {
  ...
  scene.value.fog = new THREE.FogExp2(0xffffff, 0.17);
  //设置场景背景颜色
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 1;

  toRaw(camera.value).position.y = 1;
  toRaw(camera.value).position.z = 1;
...
const initCube = () => {
  ...
  gltfLoader.load('./files/gltf/kira.glb', (gltf) => {
    ...
    gltf.scene.traverse((item) => {
      ...
      if (item.name == 'lowerarm_l') {
        meshes.lowerarm_l = item;
      }
      if (item.name == 'Upperarm_l') {
        meshes.Upperarm_l = item;
      }
      if (item.name == 'hand_l') {
        meshes.hand_l = item;
      }
      if (item.name == 'target_hand_l') {
        meshes.target_hand_l = item;
      }
      if (item.name == 'boule') {
        meshes.sphere = item;
      }
      if (item.name == 'Kira_Shirt_left') {
        meshes.kira = item;
      }
    ...
    cameraController.value.target.copy(meshes.sphere.position);

    // 关键代码手与球相连
    meshes.hand_l.attach(meshes.sphere);

    // mirror
    const cubeRenderTarget = new THREE.WebGLCubeRenderTarget(1024);
    mirrorSphereCamera = new THREE.CubeCamera(0.05, 50, cubeRenderTarget);

    scene.value.add(mirrorSphereCamera);
    const mirrorSphereMaterial = new THREE.MeshBasicMaterial({
      envMap: cubeRenderTarget.texture,
    });
    meshes.sphere.material = mirrorSphereMaterial;
    ...
    transformControls.attach(meshes.target_hand_l);
    scene.value.add(transformControls);

    // 处理点击
    transformControls.addEventListener('mouseDown', () => {
      cameraController.value.enabled = false;
    });

    transformControls.addEventListener('mouseUp', () => {
      cameraController.value.enabled = true;
    });
    console.log(meshes);
    ...
    // iks
    IKSolover = new CCDIKSolver(meshes.kira, iks);
    const ccdHelper = new CCDIKHelper(meshes.kira, iks, 0.01);
    scene.value.add(ccdHelper);
    ...
    gui = new GUI();
    gui.add(conf, 'followSphere');
    gui.add(conf, 'turnHead');
    gui.add(conf, 'ik_solver').name('IK auto update');
    gui.add(IKSolover, 'update');
  });
};
//初始化圆柱体
const initCylinder = () => {
...
const initLight = () => {
  //添加环境光
  ambientLight.value = new THREE.AmbientLight(0xffffff, 0.8);
  scene.value.add(ambientLight.value);
...
const initController = () => {
  ...
  cameraController.value.minDistance = 0.2;
  cameraController.value.maxDistance = 1.5;
...
const render = () => {
  ...
  //   相当于无线循环render
  if (mirrorSphereCamera && meshes.sphere) {
    meshes.sphere.visible = false;
    meshes.sphere.getWorldPosition(mirrorSphereCamera.position);
    mirrorSphereCamera.update(toRaw(renderer.value), toRaw(scene.value));
    meshes.sphere.visible = true;
  }

  if (meshes.sphere && conf.followSphere) {
    meshes.sphere.getWorldPosition(v0);
    cameraController.value.target.lerp(v0, 0.1);
  }

  if (meshes.head && meshes.sphere && conf.turnHead) {
    meshes.sphere.getWorldPosition(v0);
    meshes.head.lookAt(v0);
    meshes.head.rotation.y = meshes.head.rotation.y + Math.PI;
  }

  if (IKSolover && conf.ik_solver) {
    IKSolover.update();
  }
