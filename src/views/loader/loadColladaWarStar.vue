...
const initScene = () => {
  ...
  scene.value.add(new THREE.GridHelper(10, 20, 0x888888, 0x444444));
};
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 15;

  toRaw(camera.value).position.y = 10;
  toRaw(camera.value).position.z = -15;
...
const initCube = () => {
  ...
  loader.load(
    './files/collada/stormtrooper/stormtrooper.dae',
    function (collada) {
      console.log(collada);
      elf = collada.scene;
      elf.traverse((item) => {
        if (item.isSkinnedMesh) {
          item.frustCulled = true;
        }
      });
      scene.value.add(elf);

      mixer = new THREE.AnimationMixer(elf);
      const animations = elf.animations;
...
const initLight = () => {
  ...
  const poinitLight = new THREE.PointLight(0xffffff, 0.8);

  camera.value.add(poinitLight);
...
const initController = () => {
  ...
  cameraController.value.screenSpacePanning = true;

  cameraController.value.minDistance = 5;
  cameraController.value.maxDistance = 40;
