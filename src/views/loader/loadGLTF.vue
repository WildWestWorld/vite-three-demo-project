...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = -1.8;

  toRaw(camera.value).position.y = 0.6;
  toRaw(camera.value).position.z = 2.7;
...
const initCube = () => {
  ...
  loader.load('./files/gltf/DamagedHelmet/glTF/DamagedHelmet.gltf', (gltf) => {
...
const initController = () => {
  ...
  cameraController.maxDistance = 10;

  //设置控制器变换的基础点
  cameraController.value.target.set(0, 0, -0.2);
};
