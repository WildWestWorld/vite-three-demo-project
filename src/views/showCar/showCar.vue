...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 6;
...
const initCube = () => {
  ...
  loader.load('./files/models/glb/bmw01.glb', (gltf) => {
    ...
    bmw.traverse((item) => {
      ...
      // 判断是不是轮胎
      if (item.isMesh && item.name.includes('wheel')) {
        wheel.push(item);
...
const initLight = () => {
  ...
  dirLight3.position.set(10, 0, 0);
  scene.value.add(dirLight3);

  const dirLight4 = new THREE.DirectionalLight(0xffffff, 1);
  dirLight4.position.set(-10, 0, 0);
  scene.value.add(dirLight4);

  const dirLight5 = new THREE.DirectionalLight(0xffffff, 1);
  dirLight5.position.set(0, 10, 0);
  scene.value.add(dirLight5);

  const dirLight6 = new THREE.DirectionalLight(0xffffff, 0.3);
  dirLight6.position.set(5, 10, 0);
  scene.value.add(dirLight6);

  const dirLight7 = new THREE.DirectionalLight(0xffffff, 0.3);
  dirLight7.position.set(0, 10, 5);
  scene.value.add(dirLight7);

  const dirLight8 = new THREE.DirectionalLight(0xffffff, 0.3);
  dirLight8.position.set(0, 10, -5);
  scene.value.add(dirLight8);

  const dirLight9 = new THREE.DirectionalLight(0xffffff, 0.3);
  dirLight9.position.set(-5, 10, 0);
  scene.value.add(dirLight9);
};
...
const initRender = () => {
  ...
  scene.value.background = new THREE.Color('#ccc');
  scene.value.environment = new THREE.Color('#ccc');
