...
const initCube = () => {
  ...
  //   arr.forEach((item) => {
  //     let texture = textureload.load(`./files/img/living/${item}.jpg`);
  ...
  const geometry = new THREE.SphereGeometry(5, 32, 32); //SphereGeometry(radius, widthSegments, heightSegments)  SphereBufferGeometry(radius, widthSeg
  let textureload = new RGBELoader();
  let texture = textureload.load(`./files/img/hdr/Living.hdr`, (texture) => {
