...
let terrainHeight = 200;
let widthSegments = 128;
let depthSegments = 128;
...
const initScene = () => {
  ...
  //设置场景背景颜色
  scene.value.background = new THREE.Color(0xbfd1e5);
};
...
const initCube = () => {
  ...
  for (let i = 0; i < depthSegments; i++) {
    for (let j = 0; j < widthSegments; j++) {
      const radius = Math.sqrt(
        Math.pow((j - w2) / w2, 2.0) + Math.pow((i - d2) / d2, 2.0)
      );
      const height = (Math.sin(radius * phaseMult) + 1) * 0.5 * hRange;
      heightData[p] = height;
      p++;
    }
  }
  ...
  for (let i = 0, j = 0, l = vertices.length; i < l; i++, j = j + 3) {
  ...
  textLoader.load('./files/textures/grid/grid.png', (texture) => {
...
const initLight = () => {
  ...
  dirLight.shadow.mapSize.x = 1024 * 2;
  dirLight.shadow.mapSize.y = 1024 * 2;
