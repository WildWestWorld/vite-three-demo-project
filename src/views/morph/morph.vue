...
const initScene = () => {
  ...
  scene.value.fog = new THREE.Fog(0xe0e0e0, 20, 100);
};

//初始化坐标轴
const initAxis = () => {
  // 创建坐标轴 里面的10 是坐标轴的长度
  axisHelp.value = new THREE.AxesHelper(2);
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = -5;

  toRaw(camera.value).position.y = 3;
  toRaw(camera.value).position.z = 10;
...
const initCube = () => {
  ...
  loader.load('./files/gltf/RobotExpressive.glb', (gltf) => {
    console.log(gltf);
    ...
    // console.log(face);
    // console.log(face.morphTargetDictionary); //对象。类似说明书，操作表情的是下面数组 {angry:0,surpised:1,sad:2}
    // console.log(face.morphTargetInfluences); //数组
    // // 改变morphTargetInfluences 的值就能操控表情
    // face.morphTargetInfluences[2] = 1;
    initGUIV2(model, clips, face);
  });
};
//初始化圆柱体
const initCylinder = () => {
...
const initGUIV2 = (model, clips, face) => {
  let gui = new GUI();
  const actionNames = [];
  // 播放一次的动作
  ...
  for (let i = 0; i < clips.length; i++) {
    ...
    if (!loopMany.includes(clip.name)) {
      action.clampWhenFinished = true; //动画结束的最后一帧
  ...
  clipFolder
    ...
    .onChange(() => {
      ...
      console.log(activeActionObject.state);
      fadeToAction(nextActionName, 0.5);
      //   actions[nextActionName].play();
    });
  ...
  for (let i = 0; i < emotes.length; i++) {
    ...
    api[name] = function () {
      ...
      mixer.addEventListener('finished', restoreState); //如果动画做完了就回复
    };
