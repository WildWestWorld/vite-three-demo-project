...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 1000;
...
const initCube = () => {
  ...
  const hibertPoints = GeometryUtils.hilbert3D(
  ...
  const spline = new THREE.CatmullRomCurve3(hibertPoints);
  ...
  for (let i = 0; i < hibertPoints.length * subdivisions; i++) {
    const t = i / (hibertPoints.length * subdivisions);
    ...
    color.setHSL(0.9, 1.0, Math.max(0, -point.x / 200) + 0.5);
    color2.push(color.r, color.g, color.b);

    color.setHSL(i / (hibertPoints.length * subdivisions), 1.0, 0.5);
    color3.push(color.r, color.g, color.b);
  }
  ...
  for (let i = 0; i < hibertPoints.length; i++) {
    const point = hibertPoints[i];

    vertices.push(point.x, point.y, point.z);

    color.setHSL(0.6, 1.0, Math.max(0, (200 - point.x) / 400) * 0.5 + 0.5);
    color1.push(color.r, color.g, color.b);

    color.setHSL(0.3, 1.0, Math.max(0, (200 + point.x) / 200) * 0.5);
    color2.push(color.r, color.g, color.b);

    color.setHSL(i / hibertPoints.length, 1.0, 0.5);
    color3.push(color.r, color.g, color.b);
  }

  geometry1.setAttribute('color', new THREE.Float32BufferAttribute(color1, 3));
  geometry2.setAttribute('color', new THREE.Float32BufferAttribute(color2, 3));
  geometry3.setAttribute('color', new THREE.Float32BufferAttribute(color3, 3));
  ...
  geometry4.setAttribute('color', new THREE.Float32BufferAttribute(color1, 3));
  geometry5.setAttribute('color', new THREE.Float32BufferAttribute(color2, 3));
  geometry6.setAttribute('color', new THREE.Float32BufferAttribute(color3, 3));
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    line.position.set(p[2][0], p[2][1], p[2][2]);
...
const render = () => {
  ...
  //     camera.value.position.x + (mouseX - camera.value.position.x) * 0.05;

  //   camera.value.position.y =
  //     camera.value.position.y + (-mouseY + 200 - camera.value.position.y) * 0.05;

  camera.value.lookAt(scene.value.position);
  const time = Date.now() * 0.0005;

  for (let i = 0; i < scene.value.children.length; i++) {
    const object = scene.value.children[i];
    if (object.isLine) {
