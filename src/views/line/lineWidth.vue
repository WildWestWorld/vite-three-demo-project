...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 60;
...
const initCube = () => {
  ...
  const ps = GeometryUtils.hilbert3D(
    new THREE.Vector3(),
    20.2,
    1,
    0,
  ...
  const spline = new THREE.CatmullRomCurve3(ps);
  const disvisions = Math.round(ps.length * 12);
  ...
  for (let i = 0; i < disvisions; i++) {
    const t = i / disvisions;
