...
const initCube = () => {
  ...
  geometry.setAttribute(
    'orientationStart',
    new THREE.InstancedBufferAttribute(new Float32Array(orientationStart), 4)
  );

  geometry.setAttribute(
    'orientationEnd',
    new THREE.InstancedBufferAttribute(new Float32Array(orientationEnd), 4)
  );
  ...
  const material = new THREE.ShaderMaterial({
    ...
    void main(){
        ...
        vPosition = vcV * (2.0 * orientation.x) + (cross(orientation.xyz,vcV) *2.0 + vPosition);
    ...
    fragmentShader: `
        ...
        void main(){
            ...
            color.r =  color.r  +  sin(vPosition.x  *  10.0 +time) * 0.5;
            gl_FragColor = color;
        }
    `,
...
const render = () => {
  ...
  cube.value.material.uniforms.time.value = time * 0.005;
  cube.value.material.uniforms.sineTime.value = Math.sin(
    cube.value.material.uniforms.time.value * 0.05
  );
