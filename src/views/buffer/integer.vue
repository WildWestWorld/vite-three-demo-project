...
const initScene = () => {
  ...
  scene.value.fog = new THREE.Fog(0x050505, 2000, 3500);
};
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 2500;
...
const initCube = () => {
  ...
  for (let i = 0; i < triangels; i++) {
    ...
    const t = i % 3;
    textureIndeices.push(t, t, t); // 0,0,0  1,1,1 2,2,2
  }
  ...
  geometry.setAttribute(
    'textureIndex',
    new THREE.Int32BufferAttribute(textureIndeices, 1)
  );

  const loader = new THREE.TextureLoader();
  const map1 = loader.load('./files/textures/crate.gif');
  const map2 = loader.load(
    './files/textures/floors/FloorsCheckerboard_S_Diffuse.jpg'
  );
  const map3 = loader.load('./files/textures/grasslight-big.jpg');
  ...
  const material = new THREE.ShaderMaterial({
    ...
    glslVersion: THREE.GLSL3, //一定要这行，不然上面outColor会报错
  });
...
const render = () => {
  ...
  cube.value.rotation.x = time * 0.25;
  cube.value.rotation.y = time * 0.5;
