...
const initAxis = () => {
  // 创建坐标轴 里面的10 是坐标轴的长度
  axisHelp.value = new THREE.AxesHelper(200);
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 1400;
...
const initCube = () => {
  ...
  for (let i = 0, i3 = 3; i < particleCount; i++, i3 += 3) {
  ...
  material = new THREE.RawShaderMaterial({
    ...
    uniforms: {
      map: {
        value: new THREE.TextureLoader().load(
          './files/textures/sprites/circle.png'
        ),
      },
      time: { value: 0.0 },
    },
    vertexShader: `
        ...
        void main(){
            ...
            mvPosition.xyz= mvPosition.xyz + position *scale;
    ...
    fragmentShader: `
        ...
        void main(){
            ...
            gl_FragColor = vec4(
                diffuseColor.xyz  *  HSLtoRGB(vec3(vScale/5.0 , 1.0,0.5)),
                diffuseColor.w
            );
  ...
  cube.value.scale.setScalar(500);
  scene.value.add(cube.value);
};
//初始化圆柱体
const initCylinder = () => {
...
const render = () => {
  ...
  cube.value.rotation.x = time * 0.2;
  cube.value.rotation.y = time * 0.2;
