...
const initCamera = () => {
  ...
  camera.value = new THREE.OrthographicCamera(-10, 10, 10, -10, 0, 100);
...
const initCube = () => {
  ...
  //   scene.value.add(new THREE.Mesh(geometry, material));

  //shaderMaterial 重要的属性
  //1.uniform 全局变量
  //2. vertextShader:顶点着色器，定义位置信息
  ...
  shaderMaterial.uniforms = uniforms; //{time:1.0} 全部变量
  shaderMaterial.vertexShader = `
    varying vec2 vUv;  //varying can be passed to fragmentShader
    void main()	{
        vUv = uv;  //uv:planeGeometry->attibutes->uv
        gl_Position = vec4( position, 1.0 ); //position:uv:planeGeometry->attibutes->position 
    }
  `;
  ...
  uniform float time; //time From uniforms
  ...
  scene.value.add(new THREE.Mesh(geometry, shaderMaterial));
};
//初始化圆柱体
const initCylinder = () => {
  //   创建对象
...
const initLight = () => {
  //添加环境光
  ambientLight.value = new THREE.AmbientLight({ intensity: 1 });
  scene.value.add(ambientLight.value);
...
const render = () => {
  ...
  uniforms['time'].value = uniforms['time'].value + 1 / 40;
