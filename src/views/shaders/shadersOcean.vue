...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 30;

  toRaw(camera.value).position.y = 30;
  toRaw(camera.value).position.z = 100;
...
const initCube = () => {
  ...
  //sun 的初值
  sunParams = {
    elevation: 2, //极角 theta角 维度
    azimuth: -160, //方位角 phi角 经度
  };

  water = new Water(new THREE.PlaneGeometry(10000, 10000), {
    waterNormals: new THREE.TextureLoader().load(
      './files/textures/water/waternormals.jpg',
      (texture) => {
  ...
  skyUniforms['rayleigh'].value = 2;//瑞利值 与光强有光
  ...
  // 物体反射
  if (renderTaget !== undefined) {
    renderTaget.dispose();
  }
  if (pmremGenerator) {
    renderTaget = pmremGenerator.fromScene(sky);
    scene.value.environment = renderTaget.texture;
  }

  //cube
  cube.value = new THREE.Mesh(
...
const initRender = () => {
  ...
  renderer.value.toneMapping = THREE.ACESFilmicToneMapping; //用于把太阳变小
...
const initGUIV2 = () => {
  ...
  const folderSun = GUIController.value.addFolder('sun');
  ...
  const folderWater = GUIController.value.addFolder('water');
  //   该属性似乎无用，与色彩有关，但我们看不出来
  folderWater
    .add(waterParams.distortionScale, 'value', 0, 240, 0.1)
    .name('distortionScale');

  folderWater.add(waterParams.size, 'value', 0, 20, 0.1).name('size');
};
const updateSun = () => {
  // 将角度变成弧度
...
const render = () => {
  ...
  cube.value.position.y = Math.sin(time) * 20 + 5;
  cube.value.rotation.x = time * 0.5;
  cube.value.rotation.z = time * 0.5;
