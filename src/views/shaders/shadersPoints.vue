...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 250;
...
const initCube = () => {
  ...
  //geometry 里面的属性
  //normal
  //positon : 240 :4*6 = 24个点, 其中24里面有8个不重叠的点
  //uv

  // uv属性和normal 属性一致才能合并成功

  boxGeometry.deleteAttribute('normal');
  boxGeometry.deleteAttribute('uv');

  //合并重叠的点
  const mergedBoxGeometry = BufferGeometryUtils.mergeVertices(boxGeometry);
  const position = mergedBoxGeometry.getAttribute('position');
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      ...
      pointTexture: {
        value: new THREE.TextureLoader().load(
          './files/textures/sprites/disc.png'
        ),
      },
    },
    vertexShader: `
        ...
        void main(){
            vColor = customColor;
            vec4 mvPosition = modelViewMatrix * vec4(position,1.0);//defined geometry
    ...
    fragmentShader: `
        ...
        void main (){
            ...
            if(gl_FragColor.a <0.9) discard;//如果Rgb的值a小于0.9 就不让他显示
