...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 75;
...
const initCube = () => {
  ...
  cube.value = new THREE.Mesh(new THREE.BoxGeometry(15, 15, 15), material2);

  scene.value.add(cube.value);
  console.log(cube.value);

  tourusKnot = new THREE.Mesh(
  ...
  new RGBELoader().load(
    './files/textures/equirectangular/quarry_01_1k.hdr',
    (textures) => {
      textures.mapping = THREE.EquirectangularReflectionMapping;
      scene.value.background = textures;
      scene.value.environment = textures;
    }
  );
};
//初始化圆柱体
...
const initController = () => {
  ...
  cameraController.value.autoRotate = true;
};
...
const initGUI = () => {
  ...
  GUIController.value.add(material, 'roughness', 0, 1, 0.1);
  GUIController.value.add(material, 'metalness', 0, 1, 0.1);
  GUIController.value
    .add(renderer.value, 'toneMappingExposure', 0, 2, 0.1)
    .name('曝光度');
...
const render = (msTime) => {
  ...
  cube.value.rotation.x = cube.value.rotation.x + 0.02;
  cube.value.rotation.y = cube.value.rotation.y + 0.03;

  tourusKnot.position.set(
    Math.cos(time + 10) * 30,
    Math.sin(time + 10) * 30,
    Math.sin(time + 10) * 30
  );
  tourusKnot.rotation.x = tourusKnot.rotation.x + 0.02;
  tourusKnot.rotation.y = tourusKnot.rotation.y + 0.03;
  cubeCamera.update(toRaw(renderer.value), toRaw(scene.value));
