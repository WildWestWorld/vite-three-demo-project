...
const initCamera = () => {
  ...
  scene.value.add(cameraPerspectiveHelper);
...
const initRender = () => {
  ...
  //  自动清除render渲染
  toRaw(renderer.value).autoClear = false;
  renderer.value.autoClear = false;
...
const render = () => {
  ...
  mesh2.position.x = 70 * Math.cos(r);
  ...
  toRaw(renderer.value).setViewport(
    window.innerWidth / 2,
    0,
    window.innerWidth / 2,
    window.innerHeight
  ...
  toRaw(renderer.value).render(scene2, cameraPerspective);
