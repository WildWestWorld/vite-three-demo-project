...
const initCamera = () => {
  ...
  for (let i = 0; i < AMOUNT; i++) {
    for (let j = 0; j < AMOUNT; j++) {
      ...
      subcamera.position.x = i / AMOUNT - 0.5;
      subcamera.position.y = 0.5 - j / AMOUNT;
  ...
  //   toRaw(camera.value).position.x = x / AMOUNT - 0.5;
  //   toRaw(camera.value).position.y = 0.5 - y / AMOUNT;
...
const initLight = () => {
  //添加环境光
  ambientLight.value = new THREE.AmbientLight(0x222244);
  scene.value.add(ambientLight.value);
...
const render = () => {
  //   创建的cube进行旋转
  toRaw(cube.value).rotation.y = toRaw(cube.value).rotation.y + 0.005;
  toRaw(cube.value).rotation.x = toRaw(cube.value).rotation.y + 0.01;
...
window.addEventListener('resize', function () {
  ...
  for (let i = 0; i < AMOUNT; i++) {
    for (let j = 0; j < AMOUNT; j++) {
      const subcamera = camera.value.cameras[AMOUNT * i + j];
