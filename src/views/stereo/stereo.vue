...
const initScene = () => {
  ...
  //载入素材
  const urls = [
    './files/textures/cube/Park3Med/px.jpg',
    './files/textures/cube/Park3Med/nx.jpg',
    './files/textures/cube/Park3Med/py.jpg',
    './files/textures/cube/Park3Med/ny.jpg',
    './files/textures/cube/Park3Med/pz.jpg',
    './files/textures/cube/Park3Med/nz.jpg',
  ];
...
const initAxis = () => {
  // 创建坐标轴 里面的10 是坐标轴的长度
  axisHelp.value = new THREE.AxesHelper(1000);
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 3200;
...
const initCube = () => {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    mesh.scale.setScalar(size, size, size);
...
const initEffect = () => {
  effect = new StereoEffect(renderer.value);
...
const render = () => {
  ...
  camera.value.position.y =
    camera.value.position.y + (-mouseY - camera.value.position.x) * 0.05;
  ...
  effect.render(toRaw(scene.value), toRaw(camera.value));
