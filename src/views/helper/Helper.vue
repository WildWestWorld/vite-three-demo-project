...
const initHelper = () => {
  //   ponitLight helper
  scene.value.add(new THREE.PointLightHelper(ambientLight.value, 15));
  ...
  scene.value.add(polarGridHelper);
};
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 400;
...
const initCube = () => {
  ...
  loader.load('./files/gltf/LeePerrySmith/LeePerrySmith.glb', (gltf) => {
    ...
    vertexNormalsHelper = new VertexNormalsHelper(model, 5);
    // scene.value.add(vertexNormalsHelper);
    model.geometry.computeTangents();
    vertexTangentsHelper = new VertexTangentsHelper(model, 5);
    scene.value.add(vertexTangentsHelper);

    scene.value.add(new THREE.BoxHelper(model));

    const wireframe = new THREE.WireframeGeometry(model.geometry);
    ...
    scene.value.add(new THREE.BoxHelper(line));
...
const initLight = () => {
  ...
  ambientLight.value = new THREE.PointLight();
  ambientLight.value.position.set(200, 100, 150);
  scene.value.add(ambientLight.value);
};
