<template>
  <div
    ...
    style="
      background-image: url('../../files/img/earth/star.jpg');
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.y = 5;
...
const initCube = () => {
  ...
  const moonGeometry = new THREE.SphereBufferGeometry(MOON_RADIUS);
  const moonMaterial = new THREE.MeshPhongMaterial({
    map: textureLoader.load('./files/textures/planets/moon_1024.jpg'),
  });
  moon = new THREE.Mesh(moonGeometry, moonMaterial); // SphereGeometry.SphereBufferGeometry(radius, widthSegments, heightSegments  //MeshPhong
  scene.value.add(moon);

  //   创建地球
  const earthGeometry = new THREE.SphereGeometry(EARTH_RADIUS, 16, 16); //SphereBufferGeometry(radius, widthSegments, heightSegments  //MeshPhong
  const earthMaterial = new THREE.MeshPhongMaterial({
    shininess: 5,
    map: textureLoader.load('./files/textures/planets/earth_atmos_2048.jpg'),
    specularMap: textureLoader.load(
      './files/textures/planets/earth_specular_2048.jpg'
    ),
    normalMap: textureLoader.load(
      './files/textures/planets/earth_normal_2048.jpg'
    ),
  });
  earth = new THREE.Mesh(earthGeometry, earthMaterial);
  earth.receiveShadow = true;
  earth.castShadow = true;
  scene.value.add(earth);

  const earthDiv = document.createElement('div');
  earthDiv.className = 'label';
  earthDiv.textContent = 'Earth';
  const earthLabel = new CSS2DObject(earthDiv); // 创建一个2Dobject, 并将其作为dom元素使用 （这样
  ...
  const moonLabel = new CSS2DObject(moonDiv); // 创建一个2Dobject, 并将其作为dom元素使用 （这样
...
const initLight = () => {
  ...
  ambientLight.value.intensity = 0.3;
  ...
  dirLight.castShadow = true; //是否启用阴影。默认为启用。 。true。 不然会造成光
...
const render = () => {
  ...
  moon.position.set(Math.sin(elapsed) * 5, 0, Math.cos(elapsed) * 5); // 旋转球
  ...
  labelRenderer.render(toRaw(scene.value), toRaw(camera.value));
