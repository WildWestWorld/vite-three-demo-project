...
const init = () => {
  ...
  renderer.value.domElement.style.touchAction = 'none';
  renderer.value.domElement.addEventListener('pointermove', function (e) {
    // console.log('执行');
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.z = 8;
...
const initCube = () => {
  ...
  loader.load('./files/models/obj/tree.obj', (object) => {
  ...
  composer.addPass(new RenderPass(toRaw(scene.value), toRaw(camera.value)));
  ...
  textureLoader.load('texture/tri_pattern.jpg', (texture) => {
...
const initGUIV2 = () => {
  ...
  updateOutline();
  GUIController.value.add(params, 'edgeStrenth', 0.01, 10).onChange((value) => {
    updateOutline();
  });

  GUIController.value.add(params, 'edgeGlow', 0.0, 1).onChange((value) => {
    updateOutline();
  });
  GUIController.value.add(params, 'edgeThickness', 1, 30).onChange((value) => {
    updateOutline();
  });
  GUIController.value.add(params, 'pulsePeriod', 0.0, 10).onChange((value) => {
    updateOutline();
  });
  GUIController.value.add(params, 'usePatternTexture').onChange((value) => {
    updateOutline();
  });

  GUIController.value
    .addColor(params, 'visibleEdgeColor', 0.01, 10)
    .onChange((value) => {
      updateOutline();
    });

  GUIController.value
    .addColor(params, 'hiddenEdgeColor', 0.01, 10)
    .onChange((value) => {
      updateOutline();
    });
};
...
const updateOutline = () => {
  outlinePass.edgeStenth = params.edgeStrenth;
...
const render = () => {
  ...
  if (raycaster) {
    ...
    let intersects = raycaster.intersectObject(toRaw(scene.value), true);
    // console.log(intersects);
    if (intersects.length > 0) {
      const selectedObeject = intersects[0].object;
      //   console.log(intersects);
      outlinePass.selectedObjects = [];
      outlinePass.selectedObjects.push(selectedObeject);
      //   console.log(outlinePass, selectedObeject);
    } else {
      outlinePass.selectedObjects = [];
    }
  }
