...
const initAxis = () => {
  // 创建坐标轴 里面的10 是坐标轴的长度
  axisHelp.value = new THREE.AxesHelper(0.1);
...
const initCamera = () => {
  ...
  //   若不设置看不到z轴
  toRaw(camera.value).position.x = 0.2;

  toRaw(camera.value).position.y = 0;
  toRaw(camera.value).position.z = 0.2;
...
const initCube = () => {
  ...
  const textures = getTexturesFromAtlasFile(
    './files/textures/cube/sun/sun_temple_stripe.jpg',
    6
  );
  const materials = [];
  ...
  skyBox.geometry.scale(1, 1, -1); //透视盒子的关键

  scene.value.add(skyBox);
};

const getTexturesFromAtlasFile = (filePath, tilesNum) => {
  const textures = [];
  for (let i = 0; i < tilesNum; i++) {
    textures[i] = new THREE.Texture();
  }
  new THREE.ImageLoader().load(filePath, (image) => {
    console.log(image);
    // 该图片是由8张图合在一起的，所以image.height *8 = image.width
    ...
    for (let i = 0; i < tilesNum; i++) {
      ...
      canvas.height = height; // 因为我们只要一张图片，所以他的高=他的宽
      canvas.width = height;
...
const initController = () => {
  ...
  cameraController.value.enableZoom = false; //关闭放大缩小
  cameraController.value.enablePan = false;
  //   cameraController.value.enableDamping = true; //增加阻尼状态

  //   cameraController.value.rotateSpeed = -0.25;

  //设置控制器变换的基础点
  cameraController.value.target.set(0, 0, 0);
};
