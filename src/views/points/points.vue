...
const initCube = () => {
  ...
  for (let i = 0; i < 10000; i++) {
    const x = 2000 * Math.random() - 1000; // 范围-1000~1000
    const y = 2000 * Math.random() - 1000; // 范围-1000~1000
    const z = 2000 * Math.random() - 1000; // 范围-1000~1000
  ...
  const texture = new THREE.TextureLoader().load(
    './files/textures/sprites/spark1.png'
  );
...
const render = () => {
  ...
  //原本的
  //   camera.value.position.x = camera.value.position.x + mouseX * 0.05;
  camera.value.position.x =
    camera.value.position.x + (mouseX - camera.value.position.x) * 0.05;

  //   原本的计算方法
  //   camera.value.position.y = camera.value.position.y + mouseY * 0.05;
  camera.value.position.y =
    camera.value.position.y + (mouseY - camera.value.position.y) * 0.05;
...
document.body.addEventListener('pointermove', (event) => {
  ...
  mouseY = event.clientY - window.innerHeight / 2; //-height/2~height/2
});
</script>
