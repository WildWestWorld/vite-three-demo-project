...
const initScene = () => {
  ...
  //设置场景背景颜色
  scene.value.background = new THREE.Color(0xaaccff);

  //   颜色选最好和天空一个颜色，形成雾天一色
  scene.value.fog = new THREE.FogExp2(0xaaccff, 0.0007);
};
...
const initCamera = () => {
  ...
  toRaw(camera.value).position.y = 100;
  toRaw(camera.value).position.z = 300;
...
const initCube = () => {
  ...
  //   geometry
  geometryPlane = new THREE.PlaneGeometry(20000, 20000);
  ...
  const texture = new THREE.TextureLoader().load(
    './files/textures/water/water.jpg'
  );
  // wrapS水平方向,
...
const initController = () => {
  // 4.控制器
  cameraController.value = new FirstPersonControls(
    camera.value,
    renderer.value.domElement
  );
  //   因为我们设置点比较大 所以得设置大一些
  cameraController.value.movementSpeed = 500;

  //  鼠标放上去 视角自己会转
  cameraController.value.lookSpeed = 0.1;
...
const render = () => {
  ...
  //不加update 控制器无法使用, 而且要加参数delta
  cameraController.value.update(delta);
