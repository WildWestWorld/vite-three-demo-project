...
const initCamera = () => {
  ...
  toRaw(camera.value).position.y = 50;
  toRaw(camera.value).position.z = 130;

  camera.value.apect = window.innerWidth / window.innerHeight;
...
const initCube = () => {
  ...
  let texture = new THREE.TextureLoader().load('./files/textures/sky.jpg');
  //   创建巨大的天空球体
  ...
  video.src = './files/textures/sky.mp4';
  video.loop = true;
  ...
  loader.load('./files/models/glb/island2.glb', (gltf) => {
    scene.value.add(gltf.scene);
  });

  const hrdLoader = new RGBELoader();
  hrdLoader.loadAsync('./files/hdr/050.hdr').then((texture) => {
